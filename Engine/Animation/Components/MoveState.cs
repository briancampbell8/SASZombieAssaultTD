...
//
{
    ...
    public class MoveState : IAnimationState
    {
        ...
        {
            MoveTime = 0f;
            DLogger.Log(LogSubsystems.Animation, LogLevel.Debug, $"MoveState: Entering move state with speed {MovementSpeed:F2}");
        }
        ...
        public void Exit()
        {
            DLogger.Log(LogSubsystems.Animation, LogLevel.Debug, $"MoveState: Exiting move state after {MoveTime:F2}s");
        }
        ...
        public IAnimationState? CheckTransitions()
        {
            ...
            if (IsJumping)
            {
                DLogger.Log(LogSubsystems.Animation, LogLevel.Debug, "MoveState: Transition condition met for JumpState");
                return _jumpState;
            }

            if (IsAttacking)
            {
                DLogger.Log(LogSubsystems.Animation, LogLevel.Debug, "MoveState: Transition condition met for AttackState");
                return _attackState;
            }
            ...
            if (!IsMoving)
            {
                DLogger.Log(LogSubsystems.Animation, LogLevel.Debug, "MoveState: Transition condition met for IdleState (movement stopped)");
                return _idleState;
            }
