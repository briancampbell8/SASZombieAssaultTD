...
//  PATH: Engine/Animation/Core/Clips/AnimationClip.cs
//  MODULE: Core
//
//  ROLE:
//      Encapsulate core engine behavior for the AnimationClip module.
//
//  RESPONSIBILITIES:
//      - Provide AddFrame() behavior for the Core subsystem.
//      - Provide AddEvent() behavior for the Core subsystem.
//      - Provide AddTag() behavior for the Core subsystem.
//      - Provide SetMetadata() behavior for the Core subsystem.
//      - Provide GetTrack() behavior for the Core subsystem.
//      - Provide AddTrack() behavior for the Core subsystem.
//      - Provide GetFrameDuration() behavior for the Core subsystem.
//      - Provide GetDebugInfo() behavior for the Core subsystem.
//      - Provide SetTransformOffset() behavior for the Core subsystem.
//      - Provide SetColorTint() behavior for the Core subsystem.
//
...
{
    /// <summary>
    /// P11-16-01: Represents a single animation clip. Stores animation data as pure data without update logic.
    /// </summary>
    public sealed class AnimationClip
    {
        private Dictionary<AnimationEnums.AnimationTrackType, AnimationTrack> _tracks;
        /// <summary>
        ...
        /// </summary>
        public Dictionary<AnimationTrack, AnimationTrack> Tracks { get; set; }
        ...
        // public Dictionary<AnimationEnums.AnimationTrackType, AnimationTrack> Tracks { get; set; }

        public AnimationClip(Dictionary<AnimationEnums.AnimationTrackType, AnimationTrack> tracks)
        {
            _tracks = tracks;
            Frames = new List<AnimationClipFrame>();
            Events = new List<AnimationEvent>();
            Tracks = new Dictionary<AnimationTrack, AnimationTrack>();
        ...
        public AnimationClip(string name, float duration, bool isLooping = false)
        {
            ...
            DLogger.Log(
                ...
                $"AnimationClip: Created '{name}' ({duration}s, looping: {isLooping})");
        }
        ...
        public void AddFrame(AnimationClipFrame frame)
        {
            ...
            DLogger.Log(
                LogSubsystems.Animation,
                LogEnums.LogLevel.Debug,
               $"AnimationClip '{Name}': Added frame at time {frame.Time}");
        }
        ...
        public void AddEvent(AnimationEvent animationEvent)
        {
            ...
            DLogger.Log(
                ...
                LogEnums.LogLevel.Debug,
                "AnimationClip: ",
                $" '{Name}': Added event '{animationEvent.EventName}' at time {animationEvent.Timestamp}");
        }
        ...
        public void SetMetadata(string key, object? value)
        {
            if (Metadata != null)
            {
                Metadata[key] = value;
                DLogger.Log(LogSubsystems.Animation, LogEnums.LogLevel.Debug,
                    "AnimationClip: ", $" '{Name}': Set metadata '{key}' = {value}");
            }
        }
        ...
        public AnimationTrack GetTrack(AnimationTrack trackType)
        {
...

        {
            ...
            DLogger.Log(
                LogSubsystems.Animation,
                LogEnums.LogLevel.Debug,
                $"AnimationClip '{Name}': Added track '{track.Name}' of type {track.Type}");
        }
    ...
    public sealed class AnimationClipFrame
    {
        ...
        public AnimationClipFrame(float time, float duration, int spriteIndex = 0)
        {
            ...
            DLogger.Log(LogSubsystems.Animation, LogEnums.LogLevel.Debug, $"AnimationFrame: Created at time {time:F3}, duration {duration:F3}s");
        }
