// ====================================================================================================
//  FILE: JumpState.cs
//  PATH: Engine/Animation/Core/State/JumpState.cs
//  SUBSYSTEM: Animation Core State
//
//  ROLE:
//      Encapsulate core engine behavior for the JumpState module.
//
//  RESPONSIBILITIES:
...

{
    ...
    public class JumpState : IAnimationState
    {
        ...
        public void Enter()
        {
            ...
            DLogger.Log(LogSubsystems.Animation, LogEnums.LogLevel.Debug, $"JumpState: Entering jump state with height {JumpHeight:F2}");
        }
        ...
        public void Exit()
        {
            JumpVelocity = 0f;
            IsJumping = false;
            DLogger.Log(LogSubsystems.Animation, LogEnums.LogLevel.Debug, $"JumpState: Exiting jump state after {JumpTime:F2}s");
        }
        ...
        public IAnimationState? CheckTransitions()
        {
            if (IsAttacking)
            {
                DLogger.Log(LogSubsystems.Animation, LogEnums.LogLevel.Debug, "JumpState: Transition condition met for AttackState");
                return _attackState;
            }
            ...
            if (IsGrounded)
            {
                ...
                {
                    DLogger.Log(LogSubsystems.Animation, LogEnums.LogLevel.Debug, "JumpState: Transition condition met for MoveState (landing while moving)");
                    return _moveState;
                }

                DLogger.Log(LogSubsystems.Animation, LogEnums.LogLevel.Debug, "JumpState: Transition condition met for IdleState (landing while idle)");
                return _idleState;
            }

            if (JumpTime >= JUMP_DURATION_THRESHOLD)
            {
                DLogger.Log(LogSubsystems.Animation, LogEnums.LogLevel.Debug, $"JumpState: Transition condition met for IdleState (jump duration exceeded {JUMP_DURATION_THRESHOLD}s)");
                return _idleState;
            }
