...
namespace SASZombieAssaultTD.Engine.Animation.Events
{
    ...
    public class AnimationEvent
    {
        ...
        public bool Trigger()
        {
            ...
            {
                DLogger.Log(
                    LogSubsystems.Animation, LogLevel.Debug, $"AnimationEvent: Event '{EventName}' ({EventId}) is disabled, not triggering");
                return false;
            }

            if (IsTriggered)
            {
                DLogger.Log(LogSubsystems.Animation, LogLevel.Debug, $"AnimationEvent: Event '{EventName}' ({EventId}) already triggered at {Timestamp:F3}s");
                return false;
            }
            ...
            DLogger.Log(LogSubsystems.Animation, LogLevel.Debug, $"AnimationEvent: Triggered event '{EventName}' ({EventId}) at {Timestamp:F3}s (trigger #{TriggerCount})");
            return true;
        }
        ...
        {
            IsTriggered = false;
            DLogger.Log(LogSubsystems.Animation, LogLevel.Debug, $"AnimationEvent: Reset event '{EventName}' ({EventId})");
        }
        ...
        private void ValidateParameters()
        {
            ...
            if (errors.Count > 0)
            {
                DLogger.Log(LogSubsystems.Animation, LogLevel.Error, $"AnimationEvent: Parameter validation failed for event '{EventName}': {string.Join(", ", errors)}");
            }

            if (warnings.Count > 0)
            {
                DLogger.Log(LogSubsystems.Animation, LogLevel.Warning, $"AnimationEvent: Parameter validation warnings for event '{EventName}': {string.Join(", ", warnings)}");
            }
        }
        ...
        public AnimationEvent Clone()
        {
            ...
            DLogger.Log(LogSubsystems.Animation, LogLevel.Debug, $"AnimationEvent: Cloned event '{EventName}' ({EventId})");
            return clonedEvent;
        }
