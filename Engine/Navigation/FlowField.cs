...
//
{
    ...
    public sealed class FlowField
    {
        ...
        public FlowField(NavigationGrid grid)
        {
            ...
            DLogger.Log(LogSubsystems.Navigation,LogLevel.Info, "FlowField: Initialized");
        }
        ...
        public async Task GenerateAsync(Vector3Int targetPosition, CancellationToken cancellationToken = default)
        {
            ...
            {
                DLogger.Log(LogSubsystems.Unknown, LogLevel.Info, "WARNING", $"FlowField: Target position {targetPosition} is out of bounds");
                return;
            }
            ...
            try
            {
                ...
                _isGenerated = true;
                DLogger.Log(LogSubsystems.Unknown, LogLevel.Info, "DEBUG", $"FlowField: Generated flow field to target {targetPosition}");
            }
            catch (OperationCanceledException)
            {
                DLogger.Log(LogSubsystems.Navigation,LogLevel.Info, "FlowField: Flow field generation was canceled");
                _isGenerated = false;
            }
            catch (Exception ex)
            {
DLogger.Log(LogSubsystems.Navigation,LogLevel.Info,"ERROR",$"FlowField: Error generating flow field: {ex.Message}");
                _isGenerated = false;
            }
        }
        ...
        public void Clear()
        {
            ResetFlowField();
            _isGenerated = false;
            DLogger.Log(LogSubsystems.Unknown, LogLevel.Info, "DEBUG", "FlowField: Cleared flow field");
        }
