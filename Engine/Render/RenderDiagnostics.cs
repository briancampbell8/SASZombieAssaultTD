...
namespace SASZombieAssaultTD.Engine.Render
{
    ...
    public sealed class RenderDiagnostics : IDisposable
    {
        ...
        internal static void Record(string v1, string v2)
        {
            ...
            // Normalize inputs to avoid null checks everywhere
            v1 = v1 ?? string.Empty;
            v2 = v2 ?? string.Empty;

            // Compose message in a predictable, concise format
            string message;
            if (v1.Length == 0)
            {
                message = v2;
            ...
            // Use a static local lock so this static method remains thread-safe without
            // depending on instance fields (which may not be static).
            object s_recordLock = new object();

            lock (s_recordLock)
            {
                try
                {
                    // Write to diagnostics sinks. Use fully-qualified names to avoid
                    // requiring additional using directives in the containing file.
                    DLogger.Log(message);
                ...
                {
                    // Swallow any exceptions to ensure diagnostics never throw during normal execution.
                }
            }
        }
    }
