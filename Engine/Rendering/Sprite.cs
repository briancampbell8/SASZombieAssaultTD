...
//
{
    ...
    public class Sprite
    {
        ...
        public Sprite(Texture2D? texture = null, Vector3? position = null, Vector3? size = null)
        {
            ...
            UpdateSizeFromTexture();
            Dlogger.Log(LogSubsystems.Rendering, LogLevel.Debug, $"Sprite: Created with texture '{_texture?.FilePath ?? "None"}' at {_position}");
        }
        ...
        public void SetTextureRegion(int x, int y, int width, int height)
        {
            ...
            OnSpriteChanged?.Invoke(this);
            Dlogger.Log(LogSubsystems.Rendering, LogLevel.Debug, $"Sprite: Set texture region ({x}, {y}, {width}, {height})");
        }
