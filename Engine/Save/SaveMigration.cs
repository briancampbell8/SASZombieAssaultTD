...
namespace SASZombieAssaultTD.Engine.Save
{
    ...
    public class SaveMigration
    {
        ...
        public SaveMigration(int targetVersion = 2, string? saveDirectory = null)
        {
            ...
            DebugLogger.Log(DiagnosticLevel.Info, "INFO", $"SaveMigration: Initialized with target version {_targetVersion}");
        }
        ...
        public SaveData? MigrateSaveData(SaveData saveData)
        {
            if (saveData == null)
            {
                DebugLogger.Log(DiagnosticLevel.Warning, "Warning", "SaveMigration: Cannot migrate null save data");
                return null;
            }

            if (saveData.Version >= _targetVersion)
            {
                DebugLogger.Log(DiagnosticLevel.Info, "INFO", $"SaveMigration: Save is already at version {saveData.Version}, no migration needed");
                return saveData;
            }
            ...
            try
            {
                ...
                DebugLogger.Log(DiagnosticLevel.Info, "INFO", $"SaveMigration: Successfully migrated from version {sourceVersion} to {_targetVersion}");
                return currentData;
            }
            catch (Exception ex)
            {
                OnMigrationFailed?.Invoke("SaveData", ex);
                DebugLogger.Log(DiagnosticLevel.Error, "ERROR", $"SaveMigration: Migration failed - {ex.Message}");
                return null;
            }
        }
        ...
        {
            DebugLogger.Log(DiagnosticLevel.Info, "INFO", $"SaveMigration: Migrating from version {fromVersion} to {toVersion}");
        ...
        private SaveData MigrateV1ToV2(SaveData saveData)
        {
            ...
            DebugLogger.Log(DiagnosticLevel.Info, "INFO", $"SaveMigration: Migrated V1 to V2 - Added {extendedData.BattlefieldProgress.Count} battlefield progress entries");
            return extendedData;
        }
        ...
        public string? CreateBackup(string filePath)
        {
            ...
            {
                DebugLogger.Log(DiagnosticLevel.Warning, "Warning", $"SaveMigration: Cannot backup non-existent file: {filePath}");
                return null;
            }
            ...
            try
            {
                ...
                DebugLogger.Log(DiagnosticLevel.Info, "INFO", $"SaveMigration: Created backup at {backupPath}");
                return backupPath;
            }
            catch (Exception ex)
            {
                DebugLogger.Log(DiagnosticLevel.Error, "ERROR", $"SaveMigration: Backup failed - {ex.Message}");
                return null;
            }
        }
        ...
        public MigrationResult MigrateAllSaves()
        {
            ...
            {
                DebugLogger.Log(DiagnosticLevel.Warning, "Warning", $"SaveMigration: Save directory does not exist: {_saveDirectory}");
                return result;
            }
            ...
            foreach (var filePath in files)
            {
                ...
                {
                    result.FailedMigrations++;
                    DebugLogger.Log(DiagnosticLevel.Error, "ERROR", $"SaveMigration: Failed to migrate {Path.GetFileName(filePath)} - {ex.Message}");
                }
            }
