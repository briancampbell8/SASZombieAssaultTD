    ...
    public abstract class BattlefieldScene : BaseScene
    {
        ...
        protected BattlefieldScene(BattlefieldType battlefieldType, string tilemapPath)
        {
            ...
            DLogger.Log(LogSubsystems.Save,
                LogLevel.Info,
                $"BattlefieldScene: Created {battlefieldType.GetDisplayName()} scene");
        }
        ...
        public override void OnEnter()
        {
            ...
            DLogger.Log(LogSubsystems.Save,
                LogLevel.Info,
                $"BattlefieldScene: " +
                $"Entering {BattlefieldTypeExtensions.GetDisplayName(BattlefieldType.SomeValue)}");
            ...
            DLogger.Log(LogSubsystems.Save,
                LogLevel.Info,
                 $"BattlefieldScene:" +
                 $" {BattlefieldTypeExtensions.GetDisplayName(BattlefieldType.SomeValue)} initialization complete");
        }
        ...
        ///</summary>
        protected virtual void SetupSpawnNodes(object dlogger)
        {
            DLogger.Log(LogSubsystems.Save,
                LogLevel.Info,

                $"BattlefieldScene: Setting up spawn nodes for" +
                $" {BattlefieldTypeExtensions.GetDisplayName(BattlefieldType.SomeValue)}");
        ...
        protected virtual void SetupExitNodes()
        {
            DLogger.Log(LogSubsystems.Save,
                LogLevel.Info,
                $"BattlefieldScene: Setting up exit nodes for" +
                $" {BattlefieldTypeExtensions.GetDisplayName(BattlefieldType.SomeValue)}");
        ...
        protected virtual void SetupCameraBounds()
        {
            DLogger.Log(LogSubsystems.Save,
                LogLevel.Info,
                $"BattlefieldScene: Setting up camera bounds for" +
                $" {BattlefieldTypeExtensions.GetDisplayName(BattlefieldType.SomeValue)}");
        ...
        public virtual void StartNextWave()
        {
            if (IsWaveActive)
            {
                DLogger.Log(LogSubsystems.Save,
                    LogLevel.Warning,
                    $"BattlefieldScene: Wave already active, cannot start new wave");
                return;
            }
            ...
            DLogger.Log(LogSubsystems.Save,
                LogLevel.Info,
                $"BattlefieldScene: Starting wave" +
                $" {CurrentWave} on {BattlefieldTypeExtensions.GetDisplayName
                (BattlefieldType.SomeValue)} with difficulty {CurrentDifficulty}");
            DLogger.Log(LogSubsystems.Save,
                LogLevel.Info,
                $"BattlefieldScene: Wave multipliers - " +
                $"Health: {difficultyScaling.HealthMultiplier:F2}, Speed: {difficultyScaling.SpeedMultiplier:F2}, Count: {difficultyScaling.CountMultiplier:F2}, ChampionSpawnRate: {difficultyScaling.ChampionSpawnRate:P2}");
        ...
        public void SetDifficulty(DifficultyScaling.DifficultyLevel difficulty)
        {
            ...
            DLogger.Log(LogSubsystems.Save,
                LogLevel.Info,
                $"BattlefieldScene: Difficulty set to {difficulty} for {BattlefieldTypeExtensions.GetDisplayName(BattlefieldType.SomeValue)}");
        }
        ...
        protected virtual void ApplyChampionSpawnRate(float championSpawnRate)
        {
            ...
            if (shouldSpawnChampion)
            {
                ...
                DLogger.Log(
                    LogSubsystems.Save,
                    LogLevel.Info,
                    $"BattlefieldScene: Champion enemy will spawn with level {championLevel}");
