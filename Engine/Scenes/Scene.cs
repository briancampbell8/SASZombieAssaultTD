...
namespace SASZombieAssaultTD.Engine.Scenes
{
    ...
    public abstract class Scene
    {
        ...
        protected Scene(string? name = null)
        {
            ...
            Dlogger.Log(LogSubsystems.Scenes, LogLevel.Debug, $"Scene: Created '{_name}'");
        }
        ...
        public bool Load()
        {
            if (_isLoaded)
            {
                Dlogger.Log("WARNING", $"Scene: '{_name}' is already loaded");
                return false;
            }
            ...
            try
            {
                ...
                if (!success)
                {
                    Dlogger.Log("ERROR", $"Scene: Failed to load data for '{_name}'");
                    return false;
                }
                ...
                Dlogger.Log("INFO", $"Scene: Loaded '{_name}' with {_entities.Count} entities");
                return true;
            }
            catch (Exception ex)
            {
                Dlogger.Log("ERROR", $"Scene: Failed to load '{_name}' - {ex.Message}");
                return false;
            }
        }
        ...
        public bool Unload()
        {
            if (!_isLoaded)
            {
                Dlogger.Log("WARNING", $"Scene: '{_name}' is not loaded");
                return false;
            }
            ...
            try
            {
                ...
                Dlogger.Log("INFO", $"Scene: Unloaded '{_name}'");
                return true;
            }
            catch (Exception ex)
            {
                Dlogger.Log("ERROR", $"Scene: Failed to unload '{_name}' - {ex.Message}");
                return false;
            }
        }
        ...
        public void Activate()
        {
            if (!_isLoaded)
            {
                Dlogger.Log("WARNING", $"Scene: Cannot activate '{_name}' - not loaded");
                return;
            }

            if (_isActive)
            {
                Dlogger.Log("WARNING", $"Scene: '{_name}' is already active");
                return;
            }
            ...
            Dlogger.Log(LogSubsystems.Scenes, LogLevel.Debug, $"Scene: Activated '{_name}'");
        }
        ...
        public void Deactivate()
        {
            if (!_isActive)
            {
                Dlogger.Log("WARNING", $"Scene: '{_name}' is not active");
                return;
            }
            ...
            Dlogger.Log(LogSubsystems.Scenes, LogLevel.Debug, $"Scene: Deactivated '{_name}'");
        }
        ...
        public virtual void Update(float deltaTime)
        {
            ...
            {
                Dlogger.Log("ERROR", $"Scene: Failed to update '{_name}' - {ex.Message}");
            }
        }
        ...
        public virtual void Render(Renderer renderer)
        {
            ...
            {
                Dlogger.Log("ERROR", $"Scene: Failed to render '{_name}' - {ex.Message}");
            }
        }
        ...
        public bool AddEntity(Entity entity)
        {
            if (entity == null)
            {
                Dlogger.Log("WARNING", "Scene: Cannot add null entity");
                return false;
            }

            if (_entities.Contains(entity))
            {
                Dlogger.Log("WARNING", $"Scene: Entity '{entity.Id}' already exists in scene '{_name}'");
                return false;
            }
            ...
            Dlogger.Log(LogSubsystems.Scenes, LogLevel.Debug, $"Scene: Added entity '{entity.Id}' to '{_name}'");
            return true;
        }
        ...
        public bool RemoveEntity(Entity entity)
        {
            ...
            if (removed)
            {
                ...
                Dlogger.Log(LogSubsystems.Save, LogLevel.Debug, $"Scene: Removed entity '{entity.Id}' from '{_name}'");
            }
        ...
        private void InitializeEntities()
        {
            ...
            Dlogger.Log(LogSubsystems.Scenes, LogLevel.Debug, $"Scene: Initializing entities for '{_name}'");
        }
