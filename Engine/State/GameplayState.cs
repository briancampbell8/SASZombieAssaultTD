...
namespace SASZombieAssaultTD.Engine.State
{
    ...
    public class GameplayState : IGameState
    {
        ...
        public void Enter()
        {
            DLogger.Log(LogSubsystems.State,LogLevel.Info, "GameplayState: Entered - Initializing gameplay systems");
        ...
        public void Exit()
        {
            DLogger.Log(LogSubsystems.State,LogLevel.Info, "GameplayState: Exited - Cleaning up gameplay systems");
        ...
        public void HandleEvent(GameEvent gameEvent)
        {
            ...
            {
                DLogger.Log(LogSubsystems.Unknown, LogLevel.Info, "DEBUG", $"GameplayState: Ignoring event {gameEvent.GetType().Name}");
            }
        }
        ...
        private void HandleGameplayInput(GameplayInputEvent gameplayInputEvent)
        {
            switch (gameplayInputEvent.Action)
            {
                ...
                DLogger.Log(LogSubsystems.State,LogLevel.Info, "GameplayState: Pause requested - transitioning to Paused state");
                _stateMachine.ChangeState(GameStateType.Paused);
                break;
                
                case GameplayAction.QuitToMenu:
                DLogger.Log(LogSubsystems.State,LogLevel.Info, "GameplayState: Quit to menu requested - transitioning to MainMenu");
                _stateMachine.ChangeState(GameStateType.MainMenu);
                break;
                
                case GameplayAction.PlayerMove:
                //Handle player movement
                DLogger.Log(LogSubsystems.Unknown, LogLevel.Info, "DEBUG", $"GameplayState: Player movement {gameplayInputEvent.Direction}");
                break;
                
                case GameplayAction.PlayerShoot:
                //Handle player shooting
                DLogger.Log(LogSubsystems.Unknown, LogLevel.Info, "DEBUG", "GameplayState: Player shooting");
                break;
                
                default:
                DLogger.Log(LogSubsystems.Unknown, LogLevel.Info, "DEBUG", $"GameplayState: Unknown gameplay action {gameplayInputEvent.Action}");
                break;
            }
        }
    }
