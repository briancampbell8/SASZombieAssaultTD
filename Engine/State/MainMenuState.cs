...
namespace SASZombieAssaultTD.Engine.State
{
    ...
    public class MainMenuState : IGameState
    {
        ...
        public void Enter()
        {
            DLogger.Log(LogSubsystems.State,LogLevel.Info, "MainMenuState: Entered - Initializing main menu");
        ...
        public void Exit()
        {
            DLogger.Log(LogSubsystems.State,LogLevel.Info, "MainMenuState: Exited - Cleaning up main menu");
        ...
        public void HandleEvent(GameEvent gameEvent)
        {
            ...
            {
                DLogger.Log(LogSubsystems.Unknown, LogLevel.Info, "DEBUG", $"MainMenuState: Ignoring event {gameEvent.GetType().Name}");
            }
        }
        ...
        private void HandleMenuInput(MenuInputEvent menuInputEvent)
        {
            switch (menuInputEvent.Action)
            {
                ...
                DLogger.Log(LogSubsystems.State,LogLevel.Info, "MainMenuState: Start Game requested - transitioning to Gameplay");
                _stateMachine.ChangeState(GameStateType.Gameplay);
                break;
                
                case MenuAction.Quit:
                DLogger.Log(LogSubsystems.State,LogLevel.Info, "MainMenuState: Quit requested - initiating shutdown");
                //In a real implementation, this would trigger application shutdown
                //For now, we'll just log the request
                break;
                
                case MenuAction.Options:
                DLogger.Log(LogSubsystems.State,LogLevel.Info, "MainMenuState: Options requested - not implemented yet");
                //Placeholder for options menu
                break;
                
                default:
                DLogger.Log(LogSubsystems.Unknown, LogLevel.Info, "DEBUG", $"MainMenuState: Unknown menu action {menuInputEvent.Action}");
                break;
            }
        }
    }
