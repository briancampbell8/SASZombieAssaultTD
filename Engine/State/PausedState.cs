...
namespace SASZombieAssaultTD.Engine.State
{
    ...
    public class PausedState : IGameState
    {
        ...
        public void Enter()
        {
            Dlogger.Log("INFO", "PausedState: Entered - Freezing gameplay and showing pause menu");
        ...
        public void Exit()
        {
            Dlogger.Log("INFO", "PausedState: Exited - Resuming gameplay and hiding pause menu");
        ...
        public void Update(float deltaTime)
        {
            ...
            Dlogger.Log(LogSubsystems.State, LogLevel.Info, "PausedState: Update called - gameplay remains frozen");
        }
        ...
        public void HandleEvent(GameEvent gameEvent)
        {
            ...
            else if (gameEvent is GameplayInputEvent gameplayInputEvent)
            {
                ...
                {
                    Dlogger.Log("INFO", "PausedState: Resume requested - transitioning back to Gameplay");
                    _stateMachine.ChangeState(GameStateType.Gameplay);
                }
                else
                {
                    Dlogger.Log(LogSubsystems.State, LogLevel.Debug, $"PausedState: Ignoring gameplay event {gameplayInputEvent.Action} while paused");
                }
            }
            else
            {
                Dlogger.Log(LogSubsystems.State, LogLevel.Info, $"PausedState: Ignoring event {gameEvent.GetType().Name} while paused");
            }
        }
        ...
        private void HandlePauseInput(PauseInputEvent pauseInputEvent)
        {
            switch (pauseInputEvent.Action)
            {
                ...
                case PauseAction.QuitToMenu:
                    Dlogger.Log("INFO", "PausedState: Quit to menu requested - transitioning to MainMenu");
                    _stateMachine.ChangeState(GameStateType.MainMenu);
                    break;

                case PauseAction.Options:
                    Dlogger.Log("INFO", "PausedState: Options requested - not implemented yet");
                    //Placeholder for options menu from pause state
                    break;

                case PauseAction.Restart:
                    Dlogger.Log("INFO", "PausedState: Restart requested - transitioning to Gameplay");
                    //In a real implementation, this would restart the current level/mission
                    _stateMachine.ChangeState(GameStateType.Gameplay);
                    break;

                default:
                    Dlogger.Log(LogSubsystems.State, LogLevel.Info, $"PausedState: Unknown pause action {pauseInputEvent.Action}");
                    break;
            }
        }
    }
