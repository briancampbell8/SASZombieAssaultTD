// =====================================================================================================
//  FILE: TC_CostStats.cs
//  PATH: Engine/Towers/TowerControl/Upgrades/Stats/TC_CostStats.cs
//  SUBSYSTEM: Towers TowerControl Upgrades Stats Subsystem
//
//  ROLE:
//      Computes deterministic statistical summaries for tower upgrade data.
//      Provides aggregate metrics used by other TowerControl upgrade subsystems.
//      Supports lifecycle operations for initialization, execution entry, and shutdown.
//
//  RESPONSIBILITIES:
//      - Initialize with a read-only list of tower upgrades.
//      - Compute deterministic statistical values (min, max, average cost).
//      - Provide access to computed statistics for downstream systems.
//      - Maintain lifecycle structure: Initialize → Execute → Shutdown.
//
//  NON-RESPONSIBILITIES:
//      - Performing cost calculations for individual upgrades.
//      - Managing persistence, file I/O, or external data loading.
//      - Validating upgrade correctness or enforcing upgrade rules.
//
//  ARCHITECTURAL NOTES:
//      - Operates strictly on provided upgrade data; does not mutate external systems.
//      - Shutdown clears internal references and computed values for deterministic teardown.
//      - Serves as a lightweight statistical layer for TowerControl upgrade subsystems.
// =====================================================================================================

using System.Collections.Generic;   using static SASZombieAssaultTD.Engine.Diagnostics.LogEnums;
using System.Diagnostics;

namespace SASZombieAssaultTD.Engine.Towers.TowerControl.Upgrades.Stats
{
    internal sealed class TC_CostStats
    {
        private IReadOnlyList<TowerUpgrade> _upgrades;

        private int _minCost;
        private int _maxCost;
        private int _averageCost;

        public void Initialize(IReadOnlyList<TowerUpgrade> upgrades)
        {
            // Trace lifecycle initialization
            Trace.WriteLine("TC_CostStats: Initialize - Binding upgrade list and computing cost statistics.");

            // Bind upgrade list
            _upgrades = upgrades;

            // Compute deterministic statistics
            ComputeStatistics();
        }

        public void Execute()
        {
            // Trace execution entry (no runtime operations required)
            Trace.WriteLine("TC_CostStats: Execute - No active runtime operations required.");
        }

        public void Shutdown()
        {
            // Trace teardown
            Trace.WriteLine("TC_CostStats: Shutdown - Clearing upgrade list and computed statistics.");

            // Clear internal state
            _upgrades = null;
            _minCost = 0;
            _maxCost = 0;
            _averageCost = 0;
        }

        private void ComputeStatistics()
        {
            // Trace computation start
            Trace.WriteLine("TC_CostStats: ComputeStatistics - Computing min, max, and average cost.");

            // Guard against null or empty list
            if (_upgrades == null || _upgrades.Count == 0)
            {
                _minCost = 0;
                _maxCost = 0;
                _averageCost = 0;

                Trace.WriteLine("TC_CostStats: ComputeStatistics - No upgrades available; all statistics set to 0.");
                return;
            }

            int min = int.MaxValue;
            int max = int.MinValue;
            int total = 0;

            // Iterate through upgrades to compute statistics
            for (int i = 0; i < _upgrades.Count; i++)
            {
                int cost = _upgrades[i].Cost;

                if (cost < min) min = cost;
                if (cost > max) max = cost;

                total += cost;
            }

            _minCost = min;
            _maxCost = max;
            _averageCost = total / _upgrades.Count;

            // Trace final computed values
            Trace.WriteLine($"TC_CostStats: ComputeStatistics - min={_minCost}, max={_maxCost}, avg={_averageCost}.");
        }

        public int GetMinCost() => _minCost;
        public int GetMaxCost() => _maxCost;
        public int GetAverageCost() => _averageCost;
    }
}
