...
using SASZombieAssaultTD.Engine.UI.StateMachine;

namespace SASZombieAssaultTD.Engine.UI.Bootstrap
{
    internal sealed class UIBootstrapResult
    {
        // -------------------------------------------------------------------------------------------------
        // Deterministic UI Runtime Roots
        // -------------------------------------------------------------------------------------------------

        internal UIStateMachine StateMachine { get; }
        internal HUDManager HUDManager { get; }
        internal TypedTextFinalizer TextFinalizer { get; }

        // -------------------------------------------------------------------------------------------------
        // Construction
        // -------------------------------------------------------------------------------------------------

        internal UIBootstrapResult(
            UIStateMachine stateMachine,
            HUDManager hudManager,
            TypedTextFinalizer textFinalizer)
        {
            StateMachine = stateMachine;
            HUDManager = hudManager;
            TextFinalizer = textFinalizer;
        }
        ...
        {
            return $"UIBootstrapResult: StateMachine={StateMachine != null}, " +
                   $"HUDManager={HUDManager != null}, " +
                   $"TextFinalizer={TextFinalizer != null}";
        }
    }
}
