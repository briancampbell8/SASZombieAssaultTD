...
File Path: Engine/UI/HUDTextElement.cs
Purpose: HUD text element for rendering text with optional shadow and outline effects.
Features:
  - Implements IHUDElement interface for HUD integration
  - Supports text positioning with X, Y coordinates and layer depth
  - Provides text scaling factor for size adjustment
  - Supports hex color format (#RRGGBB or #AARRGGBB) for text color
  - Includes ManualTextColor override for programmatic color control
  - Provides optional drop shadow effect with configurable opacity
  - Provides optional text outline effect in 4 directions
  - Supports JSON configuration for element properties
  - Includes ParseColor method for hex color parsing with fallback
*/

using SASZombieAssaultTD.Engine.Diagnostics;

         //          Supports JSON configuration and implements IHUDElement internal interface.
