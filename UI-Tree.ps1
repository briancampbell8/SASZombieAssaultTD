...
# FILE: UI-Tree.ps1
# DESC:
#     Generates a full directory tree listing for the Engine/UI folder
#     and writes it to UI-Tree.md with fenced code blocks.
# ===================================================================================================

$root = Get-Location
$uiPath = Join-Path $root "Engine\UI"
$output = Join-Path $root "UI-Tree.md"

if (-Not (Test-Path $uiPath)) {
    Write-Host 'Engine/UI folder not found at:' $uiPath
    exit
}

# Capture tree output for Engine/UI folder only
$tree = tree $uiPath /f
